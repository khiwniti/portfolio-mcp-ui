...
/** Top technologies by number of repos that have a USES edge to them. */
export async function kgTopTechnologies(
  limit = 20
): Promise<KgEnrichment<{ slug: string; label: string; reposUsing: number }[]>> {
  if (!kgConfigured()) return { ok: false, reason: "Neo4j not configured." };
  const start = Date.now();
  const result = await runReadCypher<{ slug: string; label: string; reposUsing: number }>(
    `
    MATCH (t:Technology)<-[:USES]-(r:Repo)
    WITH t, count(DISTINCT r) AS reposUsing
    WHERE reposUsing > 0
    RETURN t.slug AS slug, coalesce(t.label, t.slug) AS label, reposUsing
    ORDER BY reposUsing DESC
    LIMIT toInteger($limit)
    `,
    { limit }
  );
  if (!result.ok) return { ok: false, reason: result.reason, tookMs: Date.now() - start };
  return { ok: true, source: "kg", data: result.records ?? [], tookMs: Date.now() - start };
}

/**
 * Top repos by number of Technology nodes they USES, with language and URL.
 * Useful for surfacing the most technology-rich repositories.
 */
export async function kgTopRepos(
  limit = 10
): Promise<KgEnrichment<{ slug: string; label: string; url: string; language: string | null; techCount: number }[]>> {
  if (!kgConfigured()) return { ok: false, reason: "Neo4j not configured." };
  const start = Date.now();
  const result = await runReadCypher<{
    slug: string;
    ...
    `
    MATCH (r:Repo)-[:USES]->(t:Technology)
    WITH r, count(DISTINCT t) AS techCount
    WHERE techCount > 1
    RETURN r.slug AS slug,
           coalesce(r.label, r.slug) AS label,
...
// ---------------------------------------------------------------------------
// Vector embedding helper (OpenAI text-embedding-3-small, optional)
// ---------------------------------------------------------------------------
 ...
 * Uses text-embedding-3-small (1536-dim) — same as the default Neo4j vector
 * index dimension when created via the portfolio KG setup scripts.
 */
...
// ---------------------------------------------------------------------------
// Full-text / vector index discovery + unified search
// ---------------------------------------------------------------------------
...
/** List all ONLINE Neo4j indexes. Returns FULLTEXT and VECTOR entries so
 *  callers can select the best available search path. */
export async function kgListIndexes(): Promise<{
  ok: boolean;
  indexes: Array<{ name: string; type: string; labelsOrTypes: string[] }>;
  reason?: string;
}> {
  const result = await runReadCypher<{
    name: string;
  ...
  if (!result.ok) return { ok: false, indexes: [], reason: result.reason };
  return {
    ok: true,
    indexes: (result.records ?? []).map((r) => ({
      name: r.name,
      type: r.type,
      labelsOrTypes: Array.isArray(r.labelsOrTypes) ? r.labelsOrTypes : [],
    })),
  };
}
 ...
 * 1. If OPENAI_API_KEY is set AND a VECTOR index exists:
 *    generate an embedding and call `db.index.vector.queryNodes`.
 * 2. If a FULLTEXT index exists: call `db.index.fulltext.queryNodes`.
 * 3. Fall back to CONTAINS substring search.
 *
 * Each tier is skipped when it returns 0 results or fails, so the search
 * always degrades gracefully to the next tier.
 *
 * Returns a normalised list of {@link KgSearchResult} plus the search mode used.
 */
export async function kgFullTextSearch(
  query: string,
  labels: string[] = [],
  limit = 20
): Promise<{
  ok: boolean;
  records?: KgSearchResult[];
  searchMode: "vector" | "fulltext" | "substring";
  ...
  const start = Date.now();
  const cap = Math.floor(Math.min(limit, 50));
  const safeLabels = labels.filter((l) => /^[A-Za-z_][A-Za-z0-9_]*$/.test(l));
  const labelClause =
    safeLabels.length > 0
      ? "AND any(label IN labels(n) WHERE label IN $labelList)"
      : "";

  // Discover indexes once upfront.
  let fulltextIndexName: string | undefined;
  let vectorIndexName: string | undefined;
  const indexResult = await kgListIndexes();
  if (indexResult.ok) {
    const byConvention = indexResult.indexes.find(
      (i) => i.type === "FULLTEXT" && i.name === "portfolioSearch"
    );
    const anyFT = indexResult.indexes.find((i) => i.type === "FULLTEXT");
    fulltextIndexName = (byConvention ?? anyFT)?.name;

    const vectorByConvention = indexResult.indexes.find(
      (i) => i.type === "VECTOR" && i.name === "portfolioVector"
    );
    const anyVec = indexResult.indexes.find((i) => i.type === "VECTOR");
    vectorIndexName = (vectorByConvention ?? anyVec)?.name;
  }
  ...
  if (vectorIndexName) {
    ...
    if (embedding) {
      const vecCypher = `
        CALL db.index.vector.queryNodes($indexName, ${cap}, $embedding)
        ...
        LIMIT ${cap}
      `;
      const vecResult = await runReadCypher<{
        labels: string[];
        title: string;
        snippet: string;
        score: number;
      ...
      if (vecResult.ok && (vecResult.records?.length ?? 0) > 0) {
        return {
          ok: true,
          records: (vecResult.records ?? []).map((r) => ({
            elementId:
              typeof r.elementId === "string" ? r.elementId : String(r.elementId ?? ""),
  ...
  if (fulltextIndexName) {
    const ftCypher = `
      CALL db.index.fulltext.queryNodes($indexName, $q, {limit: ${cap}})
      YIELD node AS n, score
      WHERE true ${labelClause}
      RETURN
        labels(n)                                                     AS labels,
      ...
      LIMIT ${cap}
    `;
    const ftResult = await runReadCypher<{
      labels: string[];
      title: string;
      snippet: string;
      score: number;
    ...
    if (ftResult.ok && (ftResult.records?.length ?? 0) > 0) {
      return {
        ok: true,
        records: (ftResult.records ?? []).map((r) => ({
          elementId:
            typeof r.elementId === "string" ? r.elementId : String(r.elementId ?? ""),
          labels: Array.isArray(r.labels) ? (r.labels as string[]) : [],
          title: typeof r.title === "string" ? r.title : String(r.title ?? ""),
  ...
  // Fall back to CONTAINS substring search.
  // Split the query into tokens so that "TypeScript backend API" matches any
  // node that contains ANY of the individual words — not just the exact phrase.
  const tokens = query
    .toLowerCase()
    .split(/\s+/)
    .map((t) => t.trim())
    .filter(Boolean);
  // Deduplicate and take the most specific tokens (longest first, max 5).
  const uniqueTokens = [...new Set(tokens)].sort((a, b) => b.length - a.length).slice(0, 5);
  ...
  const subCypher = `
    ...
    WHERE (
      any(tok IN $tokens WHERE
        ...
        toLower(coalesce(n.login, ""))       CONTAINS tok
      )
    )
    ...
    LIMIT ${cap}
  `;
  const subResult = await runReadCypher<{
    labels: string[];
    title: string;
    snippet: string;
    score: null;
  ...
  return {
    ok: true,
    records: (subResult.records ?? []).map((r) => ({
      elementId:
        typeof r.elementId === "string" ? r.elementId : String(r.elementId ?? ""),
      labels: Array.isArray(r.labels) ? (r.labels as string[]) : [],
      title: typeof r.title === "string" ? r.title : String(r.title ?? ""),
